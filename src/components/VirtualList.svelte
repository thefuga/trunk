...
import {
	createDebugInfo,
	shouldShowDebugInfo,
} from "./virtual-list/utils/virtualListDebug.js";
...
const handleHeightChangesScrollCorrection = (
	...
	if (isScrolling) {
		...
		if (pending !== 0) {
			...
			const prev = (
				heightManager.viewport as unknown as Record<string, number>
			)[key as string] as number | undefined;
			(heightManager.viewport as unknown as Record<string, number>)[
				key as string
			] = (prev ?? 0) + pending;
		}
		...
		(result) => {
			...
			if (heightManager.isReady && heightManager.initialized) {
				...
				if (Math.abs(deltaTotal) > 1) {
					...
					const currentScrollTop = heightManager.viewport.scrollTop;
					const isAtBottom =
						Math.abs(currentScrollTop - maxScrollTop) <= tolerance;
					if (isAtBottom) {
...
const visibleItems = $derived.by((): SvelteVirtualListPreviousVisibleRange => {
	...
	const scrollDelta = Math.abs(
		heightManager.scrollTop - lastProcessedScrollTop,
	);
...
export const scroll = async (
	options: SvelteVirtualListScrollOptions,
): Promise<void> => {
