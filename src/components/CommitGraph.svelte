...
const visibleColumns = $derived(ORDERED_COLUMNS.filter((k) => columnVisibility[k]));
...
const searchCurrentOid = $derived(
  searchResults.length > 0 ? (searchResults[searchCurrentIndex]?.oid ?? null) : null,
);
...
async function handleCherryPick(commit: GraphCommit) {
  ...
  try {
    ...
    const err = e as TrunkError;
    await message(
      err.message ?? "Cherry-pick failed. You may need to resolve conflicts manually.",
...
async function handleRevert(commit: GraphCommit) {
  ...
  try {
    ...
    const err = e as TrunkError;
    await message(err.message ?? "Revert failed. You may need to resolve conflicts manually.", {
      title: "Revert Error",
      kind: "error",
    });
  }
...
async function handleReset(commit: GraphCommit, mode: "soft" | "mixed" | "hard") {
  ...
  const confirmed = await ask(`Reset current branch to this commit?\n\n${labels[mode]}`, {
    title: `Reset (${mode})`,
