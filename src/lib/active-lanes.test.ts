...
describe('buildGraphData', () => {
  ...
  describe('connections (per-parent)', () => {
    it('linear chain: one connection per parent link', () => {
      ...
      const commits = [
        makeCommit({
          oid: 'A', column: 0, color_index: 0, parent_oids: ['B'],
          edges: [makeEdge({ edge_type: 'Straight', to_column: 0 })],
        }),
        makeCommit({
          oid: 'B', column: 0, color_index: 0, parent_oids: ['C'],
          edges: [makeEdge({ edge_type: 'Straight', to_column: 0 })],
        }),
        makeCommit({ oid: 'C', column: 0, color_index: 0 }),
      ];
      ...
      expect(result.connections[0]).toEqual({
        childX: 0, childY: 0, parentX: 0, parentY: 1,
        colorIndex: 0, dashed: false,
      });
      expect(result.connections[1]).toEqual({
        childX: 0, childY: 1, parentX: 0, parentY: 2,
        colorIndex: 0, dashed: false,
      });
    });
    ...
    it('merge commit produces two connections with correct colors', () => {
      ...
      const commits = [
        makeCommit({
          oid: 'M', column: 0, color_index: 0, is_merge: true,
          parent_oids: ['A', 'B'],
          edges: [
      ...
      expect(result.connections).toContainEqual({
        childX: 0, childY: 0, parentX: 1, parentY: 2,
    ...
    it('fork: cross-column connection uses commit color_index as fallback', () => {
      ...
      const commits = [
        makeCommit({
          oid: 'B0', column: 1, color_index: 2, is_branch_tip: true,
          parent_oids: ['P'],
          edges: [makeEdge({ edge_type: 'Straight', from_column: 1, to_column: 1, color_index: 2 })],
        }),
      ...
      expect(result.connections[0]).toEqual({
        childX: 1, childY: 0, parentX: 0, parentY: 1,
        colorIndex: 2, dashed: false,
      });
    });
    ...
    it('parent not loaded: connection is skipped', () => {
      const commits = [
        makeCommit({
          oid: 'A', column: 0, parent_oids: ['not_loaded'],
          edges: [makeEdge({ edge_type: 'Straight', to_column: 0 })],
        }),
      ];
      const result = buildGraphData(commits, 1);
  ...
  describe('WIP handling', () => {
    ...
    it('produces single dashed connection from WIP to HEAD row', () => {
      ...
      const wipConns = result.connections.filter(c => c.dashed && c.childY === 0);
  ...
  describe('stash splitting', () => {
    it('WIP dashed connection splits around stash in same column', () => {
      const commits = [
        makeCommit({ oid: '__wip__', column: 0, color_index: 0 }),
        makeCommit({ oid: 'stash1', column: 0, color_index: 0, is_stash: true }),
        makeCommit({ oid: 'head', column: 0, color_index: 0, is_head: true }),
      ];
      const result = buildGraphData(commits, 1);
    ...
    it('stash in different column does not split WIP connection', () => {
      const commits = [
        makeCommit({ oid: '__wip__', column: 0, color_index: 0 }),
        makeCommit({ oid: 'stash1', column: 1, color_index: 1, is_stash: true }),
        makeCommit({ oid: 'head', column: 0, color_index: 0, is_head: true }),
      ];
      const result = buildGraphData(commits, 2);
  ...
  describe('stash node', () => {
    ...
    it('stash connections are always dashed', () => {
      const commits = [
        makeCommit({
          oid: 'stash1', column: 1, color_index: 2,
          is_branch_tip: true, is_stash: true,
          parent_oids: ['parent'],
          edges: [
