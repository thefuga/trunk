...
$effect(() => {
	(async () => {
		...
		activeTabId =
			restoredTabs.find((t) => t.id === savedActive)?.id ?? restoredTabs[0].id;
	})();
});
...
// Pane persistence
$effect(() => {
	getLeftPaneWidth().then((w) => {
		leftPaneWidth = w;
	});
	getRightPaneWidth().then((w) => {
		rightPaneWidth = w;
	});
	getLeftPaneCollapsed().then((c) => {
		leftPaneCollapsed = c;
	});
