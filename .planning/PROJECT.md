...
- **Current state**: Phase 61 complete — syntax highlighting added via syntect crate. Backend `syntax` module maps base16-ocean.dark theme colors to 7 semantic CSS classes (`syn-keyword`, `syn-string`, `syn-comment`, etc.), auto-detects language from file extension, and merges syntax tokens with word-diff spans into unified `MergedSpan` arrays via sweep-line algorithm. Frontend renders merged spans with `.syn-*` CSS classes referencing `--color-syn-*` custom properties (VS Code Dark+ palette), with opacity 0.7 desaturation on add/delete line backgrounds. `DiffLine` now carries `spans: Vec<MergedSpan>` replacing separate `word_spans`/`syntax_tokens`. ~13,800 LOC TypeScript/Svelte, ~10,000 LOC Rust. Full testing infrastructure: 167+ Rust integration tests (GOOS harness), 378 frontend unit tests (vitest), 18 serde round-trip tests, 14 multi-step workflow tests, 4 watcher integration tests, 10 E2E tests (WebdriverIO). Criterion benchmarks for 7 operations + IPC round-trip + startup sequence with CI regression detection. Coverage reporting via cargo-llvm-cov + @vitest/coverage-v8.
...
---
*Last updated: 2026-03-29 after Phase 61 (Syntax Highlighting) completed*
