...
- **Current state**: Phase 60 complete — word-level diff highlighting implemented using `similar` crate. Backend pairs Delete/Add lines within hunks and populates `word_spans` byte offsets via `iter_inline_changes()`. Performance guards skip lines >500 chars or pairs with >60% edit distance. Frontend renders `<span>` elements with `.word-add`/`.word-delete` CSS classes using theme custom properties. ~13,600 LOC TypeScript/Svelte, ~9,700 LOC Rust. Full testing infrastructure: 167 Rust integration tests (GOOS harness), 375 frontend unit tests (vitest), 18 serde round-trip tests, 14 multi-step workflow tests, 4 watcher integration tests, 10 E2E tests (WebdriverIO). Criterion benchmarks for 7 operations + IPC round-trip + startup sequence with CI regression detection. Coverage reporting via cargo-llvm-cov + @vitest/coverage-v8.
...
---
*Last updated: 2026-03-28 after Phase 59 (Backend Data Model & Diff Options) completed*
